			...
			$('#add').click(function() {  
				...
				$('#'+fid+" #select2 option").each(function()
				{
					...
					tproduk = tproduk + parseFloat(jumlah);
		...
		<div class="intro-y flex flex-col sm:flex-row items-center mt-8">
                            ...
                            <tr>
                                ...
                                <td class="border-b w-5">
                                    <div class="flex sm:justify-center items-center">
                                    	  <a type="button" data-toggle="modal" data-target="#modal-edit<?=$data->id_po;?>" class="flex items-center mr-3" style="cursor: pointer;"><i data-feather="check-square" class="w-4 h-4 mr-1"></i> Ubah</a>
                                       
                                        <a class="flex items-center text-theme-6" href="<?php echo site_url('admin/po/hapus_po/'.$data->kode_po); ?>" onclick="return confirm('Apakah Anda Ingin Menghapus Data <?=$data->kode_po;?> ?');" class="btn btn-danger btn-circle" data-popup="tooltip" data-placement="top" title="Hapus Data"> <i data-feather="trash-2" class="w-4 h-4 mr-1"></i> Hapus </a>
                                    </div>
                                </td>
                            </tr>
                        <?php } ?>
			...
			<div class="modal fade" id="modal-tambah" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
              ...
              <div class="col-span-12 sm:col-span-12">
				...
				<select class="input w-full border mt-2 flex-1" name="id_sales" required>
					
					<?php foreach ($sales as $s) { ?>
					<option value="<?php echo $s->id_sales?>" selected><?php echo $s->nama_sales?></option>
					<?php } ?>
				</select>
				</div>	 
				...
				<select class="input w-full border mt-2 flex-1" multiple="true" id="select1">
					...
					<option value="<?php echo $ps->kode_produk?>_<?php echo $ps->jumlah?>_<?php echo $ps->harga?>"><?php echo $ps->nama_psales?> <?php echo $ps->jumlah?> (<?php echo $ps->satuan?>) - Rp. <?php echo number_format($ps->harga,'0','.','.') ?></option>
					<?php } ?>
				...
				<input type="text" name="total_produk" class="input w-full border mt-2 flex-1" id="tproduk" readonly>
				</div>
				...
				<div class="col-span-12 sm:col-span-12">
				<label align="left">Kategori Pembayaran</label>
				<select class="input w-full border mt-2 flex-1" name="kategori">
			...
			<div class="modal fade" id="modal-edit<?=$data->id_po;?>" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
				...
				<select class="input w-full border mt-2 flex-1" multiple="true" id="select1_<?=$data->id_po;?>">
					<?php foreach ($psales as $pd) {
						...
						foreach($data->detail as $v){
							if($pd->kode_produk == $v->kode_produk) $check = false;
						}
						if($check){
						?>
					<option value="<?php echo $pd->kode_produk?>_<?php echo $pd->jumlah?>_<?php echo $pd->harga?>"><?php echo $pd->nama_psales?> <?php echo $pd->jumlah?> (<?php echo $pd->satuan?>) - Rp. <?php echo number_format($pd->harga,'0','.','.') ?></option>
						<?php } } ?>
				...
				<select class="input w-full border mt-2 flex-1" name="kategori">
					<option value="">Pilih</option>
					<option value="<?php echo $data->kategori?>" selected><?php echo $data->kategori?> (Data Saat Ini)</option>
					<option value="KREDIT"> KREDIT</option>
					<option value="CASH"> CASH </option>
				...
				<div class="col-span-12 sm:col-span-12 align text-right">
				  <button type="button" class="button w-24 border text-gray-700 mr-1" data-dismiss="modal">Tutup</button>
                   <button type="button" onclick="submitEdit(this);" class="button w-24 bg-theme-1 text-white">Simpan</button>
				</div>
			...
			function submitEdit(el){
				var fid = $(el).closest("form").attr('id');
				var arr = fid.split('_');
				$('#'+fid+' #select2_'+arr[2]+' option').prop('selected', true);
